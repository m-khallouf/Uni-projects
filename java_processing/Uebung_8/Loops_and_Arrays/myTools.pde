 ...
 * @return String      der gewünschte Füllstring in der gewünschten Länge
 */
public String fillString(int len, String fillString, String anyText, boolean bWithQuotationMarks) {
  String result = "";
  if (len < 0)
    return result;
  
  if (anyText == null) {
    if(bWithQuotationMarks){
      result = "\"null\"" + new String(new char[(len+2)-6]).replace("\0", fillString);
      return result;
    } else {
      result = "null" + new String(new char[(len+2)-4]).replace("\0", fillString);
      return result;      
    }
  } else {
    if(bWithQuotationMarks){
      if(len - anyText.length() - 2 > 0){
        result = "\"" + anyText + "\"" + new String(new char[(len+2) - anyText.length() - 2]).replace("\0", fillString);
      }
      else {
        if(len - anyText.length()>= 0)
          result = "\"" + anyText + "\"" + new String(new char[len - anyText.length()]).replace("\0", fillString);
        else
          result = "\"" + anyText + "\"" ;
      }
      return result;
      ...
      if(len - anyText.length()>= 0)
        ...
        result = anyText + new String(new char[anyText.length()]).replace("\0", fillString);
      return result;      
    }
...
public void okOrError(boolean bErgebnis) {
  if (bErgebnis)
    ...
    println("FEHLER! FEHLER! FEHLER!");
    //System.err.println("Fehler"); // führt teilweise zu asynchronen Ausgaben (strange)
}
